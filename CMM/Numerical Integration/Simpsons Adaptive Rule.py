...
def adaptive_simpson( f, a, b, tol ):
    ...
    # Tolerance factor for conservative error control
    tol_factor = 10

    # Half of the interval length
    h = 0.5 * (b - a)

    # Define the five points for Simpson's rule
    x0 = a
    x1 = a + 0.5 * h
    x2 = a + h
    x3 = a + 1.5 * h
    ...
    # Evaluate the function at the five points
    f0 = f(x0)
    f1 = f(x1)
    f2 = f(x2)
    f3 = f(x3)
    ...
    # Simpson's rule approximations with different step sizes
    s0 = h * (f0 + 4.0 * f2 + f4) / 3.0
    s1 = h * (f0 + 4.0 * f1 + 2.0 * f2 + 4.0 * f3 + f4) / 6.0

    # Check if the error is within the tolerance
    if abs(s0 - s1) >= tol_factor * tol:
        # If not, recursively apply the adaptive Simpson's rule to the left and right halves of the interval
        s = adaptive_simpson(f, x0, x2, 0.5 * tol) + adaptive_simpson(f, x2, x4, 0.5 * tol)
    else:
        # If within tolerance, use Richardson extrapolation to improve accuracy
        s = s1 + (s1 - s0) / 15.0

    # Return the result of the integral
    return s

# Example usage of adaptive Simpson's rule
