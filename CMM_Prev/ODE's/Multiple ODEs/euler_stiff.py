""""
This code uses the Euler method to solve an ordinary differential equation (ODE) with a variable
step size. The ODE represents the equation dy/dx = -1000.0y + 3000.0 - 2000.0math.exp(-x). The solution
is computed over the interval [0, 0.3], with a change in the step size at x = 0.01 (specified by x2).
The step size is initially set to h1 for x < x2 and then increased to h2 for x > x2.
"""
